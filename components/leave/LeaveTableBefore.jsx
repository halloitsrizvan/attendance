...
"use client";
  ...
  function LeaveStatusTable({ classData: initialClassData, onDataUpdate, getLeaveStatus }) {
    const teacher = useMemo(() => {
      ...
      try {
        const stored = getSafeLocalStorage().getItem("teacher");
    ...
    return (
      <div className="min-h-64 bg-gray-100 p-4 sm:p-8 font-sans">
        <div className="max-w-2xl mx-auto space-y-4">
          {classData.length === 0 ? (
            <div className="text-center py-8">
              <p className="text-gray-500 text-lg">No pending actions found.</p>
            </div>
          ) : (
            classData.map((item) => (
              <ClassCard 
