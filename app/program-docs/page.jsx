...
const ProgramDocsPage = () => {
    ...
    const fetchLeaves = async () => {
        ...
        try {
            ...
            const docsOnly = res.data.filter(leave => leave.programDocumentUrl || leave.isProgramSubmitted);
            setLeaves(docsOnly);
        } catch (err) {
    ...
    const handleApprove = async (id) => {
        try {
            await axios.patch(`${API_PORT}/leave/${id}`, {
                programDocumented: true
            });
            alert("Program document approved successfully!");
            fetchLeaves();
        } catch (err) {
    ...
    const filteredLeaves = leaves.filter(leave => {
        ...
        const progCode = leave.programCode || "";
        
        const matchesSearch = studentName.toLowerCase().includes(searchTerm.toLowerCase()) || 
                             adNo.includes(searchTerm);
        
        const matchesCode = progCode.toLowerCase().includes(codeSearch.toLowerCase());
        
        const matchesFilter = filterStatus === 'all' || 
                             (filterStatus === 'approved' && leave.programDocumented) || 
                             (filterStatus === 'pending' && !leave.programDocumented);
                ...
                <div className="mb-8 flex flex-col md:flex-row md:items-center justify-between gap-6">
                    <div>
                        ...
                        <p className="text-slate-400 font-bold uppercase text-[11px] tracking-widest mt-2 px-1">
                            Review and approve OGEA program documentation
                        </p>
                    </div>
                    ...
                    <div className="flex flex-col lg:flex-row gap-4 w-full lg:w-auto">
                        <div className="flex flex-col sm:flex-row bg-white rounded-[1.5rem] border border-slate-200 shadow-sm overflow-hidden w-full lg:w-auto">
                            ...
                            <div className="relative flex-1 sm:w-48">
                                ...
                                <input 
                                    ...
                                    className="w-full pl-12 pr-4 py-4 text-sm font-black text-purple-600 uppercase tracking-widest focus:outline-none placeholder:text-slate-300 placeholder:normal-case placeholder:tracking-normal"
                                />
                            </div>
                        </div>
                        ...
                        <select 
                            ...
                            className="w-full lg:w-auto px-6 py-4 bg-white border border-slate-200 rounded-[1.5rem] text-sm font-bold text-slate-700 focus:border-purple-500 outline-none transition-all shadow-sm cursor-pointer appearance-none"
                        >
                    ...
                    {loading ? (
                                        ...
                                        <tr key={leave._id} className="hover:bg-slate-50/30 transition-colors group">
                                            <td className="px-8 py-6">
                                                <div className="flex items-center gap-4">
                                                    <div className="w-12 h-12 bg-slate-100 rounded-2xl flex items-center justify-center text-slate-500 group-hover:bg-purple-100 group-hover:text-purple-600 transition-all font-black text-sm shadow-inner">
                                            ...
                                            <td className="px-8 py-6">
                                                <div className="space-y-1">
                                                    ...
                                                    <div className="flex flex-col gap-1 text-[9px] font-black uppercase tracking-widest">
                                                        ...
                                                        {leave.returnedAt && (
                                                            <div className="flex items-center gap-1.5 text-purple-500 mt-0.5 pt-0.5 border-t border-slate-100">
                                                                <Clock size={10} /> 
                                            ...
                                            <td className="px-8 py-6">
                                                {leave.programDocumentUrl ? (
                                                    <a 
                                                        ...
                                                        className="inline-flex items-center gap-2 px-4 py-2 bg-purple-50 text-purple-600 rounded-xl text-[10px] font-black uppercase tracking-widest border border-purple-100 hover:bg-purple-600 hover:text-white transition-all shadow-sm"
                                                    >
                                            ...
                                            <td className="px-8 py-6 text-center">
                                                <span className={`px-4 py-1.5 rounded-full text-[9px] font-black uppercase tracking-widest shadow-sm inline-flex items-center gap-1.5
                                                    ${leave.programDocumented ? 'bg-emerald-500 text-white shadow-emerald-200' : 'bg-amber-500 text-white shadow-amber-200'}`}>
                                                    {leave.programDocumented ? <CheckCircle size={10} /> : <Clock size={10} />}
                                                    {leave.programDocumented ? 'Verified' : 'Pending'}
                                                </span>
                                            </td>
                                            <td className="px-8 py-6 text-right">
                                                {!leave.programDocumented ? (
                                                    <button 
                                                        onClick={() => handleApprove(leave._id)}
                                                        className="px-6 py-2.5 bg-slate-900 text-white rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-purple-600 active:scale-95 transition-all shadow-lg shadow-slate-200"
                                                    >
                                                        Approve
                                                    </button>
                                                ) : (
                        ...
                        <div className="h-96 flex flex-col items-center justify-center text-center p-8">
                            ...
                            <p className="text-[10px] font-bold text-slate-300 uppercase tracking-widest mt-2 max-w-xs mx-auto">
                                No student has uploaded any program documentation matching your criteria.
                            </p>
                        </div>
                    )}
                </div>
