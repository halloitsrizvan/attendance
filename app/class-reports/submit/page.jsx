...
import { BookOpen, Calendar, CheckCircle2, Clock, PlusCircle, Edit3, Trash2, X, Trophy, Image as ImageIcon, ImagePlus, Loader2, ChevronLeft, ChevronDown, AlertTriangle } from 'lucide-react';
...
export default function SubmitClassReport() {
    ...
    useEffect(() => {
        ...
        if (storedTeacher) {
            ...
            fetchReports(parsedTeacher.classNum);
        } else {
            setLoading(false);
        }
    }, []);
    ...
    const fetchReports = async (classNum) => {
        ...
        try {
            const reportsRes = await axios.get(`/api/class-reports?classNumber=${classNum}`);
    ...
    const handleUpdateProgram = async (e, finalize = false) => {
        ...
        if (editForm.tier === 'Tier 1') {
            try {
                ...
                if (programToEdit && programToEdit.tier !== 'Tier 1') {
                    const existingRes = await axios.get(`/api/class-reports?classNumber=${teacher.classNum}`);
        ...
        try {
            ...
            await axios.put(`/api/class-reports/${editingReportId}`, {
    ...
    return (
        <div className="min-h-screen bg-slate-50 pb-20">
            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 pt-24 space-y-6">
                ...
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-8">
                    <div>
                        ...
                        <p className="text-slate-500 font-bold uppercase text-[11px] tracking-[0.2em]">
                            MANAGE MONTHLY PROGRAM REPORTS FOR CLASS {teacher.classNum}
                        </p>
                    </div>
                    <button
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                                    {report.programs.map(program => (
                                        <div key={program._id} className="bg-slate-50/40 border border-slate-200/60 rounded-[1.5rem] p-5 sm:p-6 flex flex-col relative group hover:bg-white hover:shadow-md transition-all duration-300">
                                            ...
                                            {report.status === 'pending' && (!report.classTeacherApproved || report.submitterType === 'teacher') && (
            ...
            {isSubmitModalOpen && (
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
                    ...
                    <div className="relative bg-white w-full max-w-4xl max-h-[90vh] flex flex-col rounded-[2.5rem] shadow-2xl overflow-hidden animate-in fade-in zoom-in duration-300 border border-slate-100 z-50">
                        {/* Modal Header */}
                        <div className="p-6 bg-indigo-600 text-white flex items-center justify-between shrink-0 relative overflow-hidden">
                            <div className="absolute top-0 right-0 w-48 h-48 bg-white/10 rounded-full -mr-20 -mt-20 blur-2xl"></div>
                            <div>
                                <h2 className="text-2xl font-black uppercase italic tracking-tight flex items-center gap-3 relative z-10">
                                    <BookOpen size={24} /> Submit New Programs
                                </h2>
                                <p className="text-indigo-200 text-xs mt-1 relative z-10 font-bold uppercase tracking-wider">
                                    Class {teacher.classNum}
                                </p>
                            </div>
                            <button onClick={() => setIsSubmitModalOpen(false)} className="p-2 hover:bg-white/20 rounded-xl transition-all relative z-10"><X size={20} /></button>
                        </div>
                        {/* Modal Body */}
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
                    ...
                    <div className="relative bg-white w-full max-w-lg max-h-[90vh] flex flex-col rounded-[2.5rem] shadow-2xl overflow-hidden animate-in fade-in zoom-in duration-300 border border-slate-100 z-50">
                        ...
                        <form className="p-6 overflow-y-auto custom-scrollbar space-y-4 bg-white">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-5 pt-4 border-t border-slate-100 mt-4">
                                ...
                                <div>
                                    ...
                                    <div className="mt-2 flex gap-2">
                                        <label className="h-[50px] flex-1 flex items-center justify-center gap-2 p-3 border-2 border-dashed border-slate-200 rounded-xl bg-slate-50 cursor-pointer hover:border-blue-300 hover:text-blue-500 transition-all text-slate-400 text-xs font-bold shadow-sm">
                                            <input
                                                type="file"
                                                multiple
                                                accept="image/*"
                            ...
                            {/* Gallery Previews in Edit Modal */}
                            {editForm.gallery && editForm.gallery.length > 0 && (
                                <div className="flex gap-2 overflow-x-auto py-2 custom-scrollbar border-t border-slate-50 mt-4">
                                    {editForm.gallery.map((url, i) => (
                                        <div key={i} className="relative w-16 h-16 rounded-lg overflow-hidden border border-slate-200 shrink-0 shadow-sm">
                                            ...
                                            <button
                                                ...
                                                className="absolute top-1 right-1 w-5 h-5 bg-black/60 hover:bg-rose-500 text-white rounded-full flex items-center justify-center transition-colors"
                                            >
                                                <X size={10} />
                                            </button>
                                        </div>
                            ...
                            {/* Action Buttons */}
                            <div className="grid grid-cols-2 gap-3 pt-6 border-t border-slate-100 mt-6">
                                {editForm.isDraft ? (
                                    <>
                                        <button
                                            type="button"
                                            onClick={(e) => handleUpdateProgram(e, false)}
                                            className="w-full py-4 bg-slate-100 hover:bg-slate-200 text-slate-700 rounded-[1.5rem] font-black uppercase text-[10px] tracking-widest transition-all active:scale-95 shadow-sm"
                                        >
                                            Save Draft
                                        </button>
                                        <button
                                            ...
                                            className="w-full py-4 bg-[#0F172A] hover:bg-slate-800 text-white rounded-[1.5rem] font-black uppercase text-[10px] tracking-widest transition-all active:scale-95 shadow-md"
                                        >
                                            Finalize & Submit
                                        </button>
                                    </>
                                    ...
                                    <button
                                        type="submit"
                                        onClick={(e) => handleUpdateProgram(e, false)}
                                        className="col-span-2 w-full py-4 bg-blue-600 hover:bg-blue-700 text-white rounded-[1.5rem] font-black uppercase text-[10px] tracking-widest transition-all active:scale-95 shadow-md"
                                    >
                                        Save Changes
                                    </button>
                                )}
